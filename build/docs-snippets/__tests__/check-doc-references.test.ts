...
describe('checkReferences', () => {
  ...
  it('reports a built file the package does not emit', () => {
    expect(problems("import 'vue3-maptiler-gl/dist/styles.css';")).toEqual([
      "error: the build emits no 'dist/styles.css'",
    ]);
  });

  it("leaves MapTiler's own dist paths alone", () => {
    expect(
      problems(
        '<script src="https://unpkg.com/@maptiler/sdk/dist/x.js"></script>',
      ),
    ).toEqual([]);
  });
