...
describe('coverageSnippet', () => {
  it('excludes every documented name from the leftover keys', () => {
    ...
    expect(code).toContain("import { useFlyTo } from 'vue3-maptiler-gl';");
  });

  it('forgives only the members of a declared spread source', () => {
    const { code } = coverageSnippet({
      file: FILE,
      composable: 'useMapTiler',
      line: 12,
      documented: ['register'],
      spreads: ['MapTilerMethods'],
    });

    // A type spread is imported as a type and excluded by its keys, so a field
    // outside it still fails.
    expect(code).toContain(
      "import type { MapTilerMethods } from 'vue3-maptiler-gl';",
    );
    expect(code).toContain("'register' | keyof MapTilerMethods");
  });
  ...
  it('resolves a composable spread through its return type', () => {
    ...
    expect(code).toContain(
      "import { useLayerEventListener, useMapEventListener } from 'vue3-maptiler-gl';",
    );
    expect(code).toContain(
...
describe('componentCoverageSnippet', () => {
  ...
  it('forgives the events a component says it shares with another', () => {
    ...
    expect(code).toContain(
      "import { CircleLayer, FillLayer } from 'vue3-maptiler-gl';",
    );
...
describe('slotSnippet', () => {
  ...
  it('reads the template slots, not the index signature Vue adds', () => {
    ...
    expect(code).toContain(
      "type Slots = Named<InstanceType<typeof MapTiler>['$slots']>;",
    );
  });
...
describe('parametersSnippet argument positions', () => {
  ...
  it('puts the object last when the function takes more than one argument', () => {
    // `useCreateMapTiler(elRef, styleRef, props)` tabulates only `props`.
  ...
  it('builds each parameter separately when a row names one', () => {
    // `useMapTiler(options)` tabulates `options` and then `options.debug`. The
