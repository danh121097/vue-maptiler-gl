...
`);
  ...
  it("reads a spread marker as the section's abridgement", () => {
    const found = tables(`### useMapTiler
...
It also spreads in every method of \`MapTilerMethods\`.

<!-- returns-spread: MapTilerMethods -->
`);

    expect(found[0]!.spreads).toEqual(['MapTilerMethods']);
  });
...
`);
    ...
    expect(found.map((t) => t.spreads)).toEqual([['MapTilerMethods'], []]);
  });
...
const COMPONENT_NAME_RE = /\b(?:MapTiler|FillLayer|CircleLayer|Popup)\b/g;
...
`;
    ...
    ).toEqual([['MapTiler', [['debug', 'boolean']]]]);
    ...
    ).toEqual([['MapTiler', [['click', 'MapMouseEvent']]]]);
  });
...
`,
    ...
    expect(found.map((t) => t.composable)).toEqual(['useCreateMapTiler']);
  });
