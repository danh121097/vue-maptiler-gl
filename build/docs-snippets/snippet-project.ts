...
const COMPILER_OPTIONS = {
  ...
  paths: {
    ...
    'vue3-maptiler-gl': ['dist/index.d.ts'],
    'vue3-maptiler-gl/maptiler': ['dist/maptiler-reexports.d.ts'],
    'vue3-maptiler-gl/components': ['dist/components/index.d.ts'],
    'vue3-maptiler-gl/composables': ['dist/composables/index.d.ts'],
 ...
 *
 * `Marker` and `Popup` at the root are this library's Vue components; MapTiler's
 * classes of those names are re-exported as `MapTilerMarker` / `MapTilerPopup`.
 * A Returns row saying `Marker` means MapTiler's, so the check has to take it
 * from MapTiler or it would compare a component against a marker and fail.
 */
...
export function tableSnippet(
  ...
  const head = [
    ...
    ...from(FROM_VUE, 'vue'),
    ...from(FROM_MAPLIBRE, '@maptiler/sdk'),
    ...from(null, 'vue3-maptiler-gl'),
    `import { ${table.composable} } from 'vue3-maptiler-gl';`,
    ...(generic
      ? [
...
export function coverageSnippet(coverage: Coverage): Snippet {
  ...
  const code = [
    ...(typeSpreads.length
      ? [
          `import type { ${typeSpreads.sort().join(', ')} } from 'vue3-maptiler-gl';`,
        ]
      : []),
    `import { ${values.join(', ')} } from 'vue3-maptiler-gl';`,
    `type Extra = Exclude<`,
...
export function documentedTypeSnippet(
  ...
  const head = [
    ...
    ...from(null, 'vue3-maptiler-gl'),
    type.kind === 'enum'
      ? `import { ${type.name} as _real } from 'vue3-maptiler-gl';`
      : `import type { ${type.name} as _real } from 'vue3-maptiler-gl';`,
  ];
...
export function attributeSnippet(
  ...
  const head = [`import { ${components.join(', ')} } from 'vue3-maptiler-gl';`];
...
export function componentCoverageSnippet(coverage: ComponentCoverage): Snippet {
  ...
  const code = [
    ...
    `  VNodeProps,`,
    `} from 'vue';`,
    `import { ${imports.join(', ')} } from 'vue3-maptiler-gl';`,
    `type Extra = Exclude<`,
...
export function parametersSnippet(
  ...
  // Which argument the object is. `useCreateMapTiler(elRef, styleRef, props)`
