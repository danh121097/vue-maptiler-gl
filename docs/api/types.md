...
Vue3 MapTiler SDK provides comprehensive TypeScript support with well-defined types and interfaces for all components and composables.
...
```typescript
type MapTilerActions = CreateMapTilerActions & {
...
```typescript
interface CreateMapTilerActions {
...
Everything [`CreateMapTilerActions`](#createmaptileractions) has, plus the map's
accessors and the remaining setters. [`useMapTiler`](/api/composables#usemaptiler)
...
```typescript
type MapTilerMethods = CreateMapTilerActions & {
...
import { GeoJsonSource, CircleLayer } from 'vue3-maptiler-gl';
import type { LayerClickHandler } from 'vue3-maptiler-gl';
...
```vue
<script setup lang="ts">
import { GeoJsonSource, FillLayer } from 'vue3-maptiler-gl';
import type { LayerMouseHandler } from 'vue3-maptiler-gl';
...
Vue3 MapTiler SDK re-exports all relevant MapTiler SDK types for convenience:
...
For complete type definitions, refer to the [MapTiler SDK documentation](https://docs.maptiler.com/sdk-js/api/).
