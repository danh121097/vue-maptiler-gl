...
Holds a map created elsewhere, so a component that renders `<MapTiler>` can
drive it without reaching into a template ref. It creates no map of its own:
hand it the actions `<MapTiler>` emits, via `@register`.
...
It also spreads in every accessor and setter of [`MapTilerMethods`](/api/types#maptilermethods)
...
`useCreateMapTiler`, which owns the map.
...
To reach the map from a component _nested inside_ `<MapTiler>`, inject
...
| `prewarmResources`         | `boolean` | `true`  | Prewarm MapTiler resources on initialization |
...
| `options` | `MarkerOptions`                     | `{}`    | Native MapTiler `Marker` options                              |
...
| Property               | Type                                                     | Description                                           |
| ---------------------- | -------------------------------------------------------- | ----------------------------------------------------- |
| `marker`               | `ComputedRef<Marker \| null>`                            | The underlying MapTiler `Marker` instance             |
...
| `options`      | `PopupOptions`                      | `{}`    | Native MapTiler `Popup` options                      |
...
| Property          | Type                             | Description                                                                 |
| ----------------- | -------------------------------- | --------------------------------------------------------------------------- |
| `popup`           | `ComputedRef<Popup \| null>`     | The underlying MapTiler `Popup` instance                                    |
...
| `type`        | `LayerTypes`                                                             | —         | MapTiler layer type (e.g. `'fill'`, `'line'`) |
