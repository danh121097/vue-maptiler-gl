...
Vue3 MapTiler GL provides a comprehensive set of composables for building interactive maps with Vue 3 Composition API. All composables are designed with TypeScript support, reactive data binding, and comprehensive error handling.

## Map Composables

### useCreateMapTiler

The core composable for creating and managing MapTiler GL Maps with enhanced error handling and reactive state management.
...
| `props`    | `CreateMapTilerProps`                    | Configuration options for the map       |

#### CreateMapTilerProps Interface

| Property   | Type                                               | Default     | Description                          |
| ---------- | -------------------------------------------------- | ----------- | ------------------------------------ |
| `register` | `(actions: EnhancedCreateMapTilerActions) => void` | `undefined` | Callback for registering map actions |
...
| Parameter | Type               | Description           |
| --------- | ------------------ | --------------------- |
| `props`   | `UseMapTilerProps` | Configuration options |
...
const { mapInstance, isReady } = useMapTiler({
  debug: true,
});
```
...
Creates and manages MapTiler GL Fill Layers with reactive updates and comprehensive event handling.
...
Creates and manages MapTiler GL Circle Layers for point data visualization.
...
Creates and manages MapTiler GL Line Layers for linear features.
...
Creates and manages MapTiler GL Symbol Layers for icons and text.
...
Creates and manages MapTiler GL GeoJSON Sources with reactive data updates and comprehensive error handling.
...
Creates and manages MapTiler GL Geolocate Controls with comprehensive event handling.
...
Provides reactive event handling for MapTiler GL map events with automatic cleanup.
...
Provides reactive event handling for MapTiler GL layer events with automatic cleanup.
...
```typescript
import { useBounds } from 'vue3-maptiler-gl';
...
```typescript
import { useZoom } from 'vue3-maptiler-gl';
