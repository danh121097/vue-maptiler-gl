...
Comprehensive reference for all 38 composables available in vue3-maptiler-gl.
...
Create and manage a MapTiler SDK instance with full lifecycle support.
...
- `props?: CreateMapTilerProps` - Configuration (center, zoom, bearing, etc.)
...
const map: {
  ...
  removeMap: () => void;
  destroyMap: () => void;
} = useCreateMapTiler(container, 'https://demotiles.maplibre.org/style.json');
```
...
Hold a map created by `<MapTiler>` so the surrounding component can drive it.
It creates no map: pass its `register` to `<MapTiler>`'s `@register`.

**Returns**: the same camera setters and map accessors as `useCreateMapTiler`,
plus `register`, `isRegistered` and `setMapOptions`. The status field is named
`mapStatus`, not `mapCreationStatus`, and there is no
`initMap` / `removeMap` / `destroyMap` — `<MapTiler>` controls the lifecycle.
...
const { register: registerMap, isMapReady, setZoom } = useMapTiler();
...
To reach the map from a component nested **inside** `<MapTiler>`, inject
`MapProvideKey` — that is what the built-in child components do.

### `useMapTilerConfig(options?)`

Set MapTiler's **global** performance settings. It configures the library, not a
...
const { clearPrewarmedResources } = useMapTilerConfig({
  workerCount: 8,
...
one — the layer counterpart to `useMapTiler`. It takes no map and no source.
...
**Event Types**: All MapTiler map events
...
Tracking is configured through `options.trackUserLocation`, which MapTiler's own
...
export them — `import { createCameraAnimation } from 'vue3-maptiler-gl'` works.
