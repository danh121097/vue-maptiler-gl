...
Republish of v6.0.0. The v6.0.0 tarball on npm was built before `@maptiler/sdk`
...
- Import MapTiler's stylesheet and runtime from their own paths
...
- Emit preserved modules and unpin the MapTiler runtime
...
- Model MapTiler's layer-scoped listener overload in the mock
...
- Set nuxt-maptiler-gl to 2.0.0 and stop guessing the bump

- Make @maptiler/sdk a peer dependency
...
### Performance
- Fixed event listener memory leak in MapTiler.vue
...
- Verified `shallowRef` + `markRaw` for all MapTiler object storage
...
- Documentation updates for MapTiler integration
