...
This document defines the code standards, conventions, and best practices used in vue3-maptiler-gl.
...
import type { Map, LayerSpecification } from '@maptiler/sdk';
...
| ---------------------- | ---------------------------------------- | ------------------------------------------------ |
| **Components**         | PascalCase                               | `MapTiler`, `GeoJsonSource`, `FillLayer`         |
| **Composables**        | camelCase with `use` prefix              | `useMapTiler`, `useFlyTo`, `useMapEventListener` |
| **Enums**              | PascalCase                               | `MapCreationStatus`, `EventListenerStatus`       |
| **Types/Interfaces**   | PascalCase                               | `MapTilerActions`, `CreateMapTilerActions`       |
...
import type { Map, MapMouseEvent } from '@maptiler/sdk';
...
transcribed. Where an emit forwards a maptiler event, its payload is
maptiler's, and the forwarding call site is written so TypeScript checks that:
assigning the overloaded `emits` to one generic signature over `MapEventType`
fails if any declared payload is not what maptiler delivers.
...
```typescript
// Use shallowRef for MapTiler objects (no unwanted Vue tracking)
...
const { mapInstance, isReady } = useCreateMapTiler(container, style);
...
```typescript
// MapTiler's own camera methods are not promises — `map.flyTo()` returns the
...
export function useMapTiler(): UseMapTilerResult {
  // Implementation
}
```
...
```typescript
// Good - Use shallowRef for MapTiler objects
const mapInstance = shallowRef<Map | null>(null);
...
- Always use `.maptiler-*` class prefix
 ...
 *
 * @param map - The MapTiler instance
...
against `dist/`, resolving `vue3-maptiler-gl` the way an installed consumer's
...
```markdown
<!-- returns-spread: MapTilerMethods -->
...
that is misspelled or not exported fails outright. `useMapTiler` spreads in all
47 accessors and setters of `MapTilerMethods`; that is the case the marker
...
`Destroyed`, `GeolocateEventTypes` naming `trackingstart` for an event MapTiler
...
`/api/types#maptilermethods` for months to a page with no such heading, and
...
That first run found the reference honest and the _library_ wrong. `MapTiler`
declared the DOM `Event` where maptiler delivers `MapTilerGLEvent`, so
...
component never forwards; and `MapTiler`, `GeoJsonSource` and
...
must name a real package script, a `vue3-maptiler-gl/dist/…` path must be a
...
```typescript
// Validate user input before passing to MapTiler
...
- Use semantic HTML
- Provide keyboard navigation (inherited from MapTiler SDK)
