...
Vue3 MapTiler SDK is a Vue 3 component library that provides reactive, type-safe bindings to MapTiler SDK. The architecture follows a layered design with clear separation of concerns:

```
User Application (Vue 3 / Nuxt)
        │
        ├─► Components Layer (MapTiler, GeoJsonSource, Layers, etc.)
        │
        ├─► Composables Layer (useMapTiler, useFlyTo, useMapEventListener, etc.)
        │
        ...
        │
        └─► MapTiler SDK (Maps, Layers, Sources, Events)
```
...
```typescript
import { ref } from 'vue';
import { useFlyTo, type Map } from 'vue3-maptiler-gl';
...
```typescript
import { ref } from 'vue';
import { useCreateFillLayer, type Map } from 'vue3-maptiler-gl';
...
```typescript
// MapTiler.vue provides map context
...
`MapProvideKey` — provided by `MapTiler`, carrying the map instance and nothing
else. Status is not injected: a descendant that needs it calls `useMapTiler()`.

```typescript
// Inject with a fallback ref, so a component used outside <MapTiler> renders
// instead of throwing.
...
```
Mount
  ├─ useCreateMapTiler initializes MapTiler instance
...
5. **markRaw on MapTiler objects** (opt-out of Vue proxy)

## Component Architecture

### MapTiler Component (Root)

**Responsibilities**:

- Initialize MapTiler instance via `useCreateMapTiler`
- Provide map context to descendants
...
- `options: Partial<MapOptions>` - MapTiler configuration
- `register?: (actions: MapTilerActions) => void` - Access map methods
...
- Create GeoJSON source in MapTiler
- Add to map on mount
...
- Wrap MapTiler SDK controls/overlays
...
#### `useCreateMapTiler(elRef, styleRef, props)`

**Purpose**: Create and manage a MapTiler instance.

**Returns**: [`useCreateMapTiler`](./api/composables.md#usecreatemaptiler) in the API reference. That page holds the only copy; this one used to keep a second, and it drifted.
...
const { mapInstance, isMapReady, setCenter, setZoom } = useCreateMapTiler(
  mapContainer,
  styleRef,
  {
    center: [0, 0],
...
**Returns**: [`useMapTiler`](./api/composables.md#usemaptiler) in the API reference. That page holds the only copy; this one used to keep a second, and it drifted.

**Usage Pattern**:

```typescript
// In any component nested under MapTiler
import { useMapTiler } from 'vue3-maptiler-gl';

const { mapInstance, isMapReady } = useMapTiler();

// Safe because useMapTiler validates context exists
```
...
```typescript
import { ref, watch } from 'vue';
import { useCreateGeoJsonSource, type Map } from 'vue3-maptiler-gl';
...
```
MapTiler SDK Types (@maptiler/sdk package)
    │
    ...
    └─► ... (50+ types)
         │
Vue3-MapTiler SDK Extended Types
    │
    ├─► MapTilerActions (component register callback)
    ├─► CreateMapTilerActions (composable return type)
...
| Strategy            | Benefit                                     |
| ------------------- | ------------------------------------------- |
| `shallowRef`        | Prevents Vue from wrapping MapTiler objects |
...
- Components use `shallowRef` for MapTiler instances
...
The `nuxt-maptiler-gl` module handles:
...
useCreateMapTiler(mapContainer, styleRef, {
...
241 tests across 35 files, run with Vitest under happy-dom. Coverage is enforced
...
Every test drives a hand-written MapTiler mock; no test runs a real map, so a
wrong assumption about MapTiler's own behaviour stays invisible until a consumer
