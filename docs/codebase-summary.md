...
│   │   │   ├── useCreateMapTiler.ts       # Create map instance
│   │   │   ├── useMapTiler.ts            # Access map context
│   │   │   ├── useMapTilerConfig.ts      # Configure map
...
│   │   └── runtime/
│   │       └── plugins/
│   │           └── maptiler-sdk.client.ts  # Client plugin
...
│   └── maptiler.d.ts              # MapTiler SDK types
...
| **Components**          | 10                               |
| **Composables**         | 38                               |
| **Unit Tests**          | 241 across 35 files              |
...
- **File**: `libs/components/MapTiler.vue`
...
- **Events**: `load`, `error`, `click`, `move`, `zoom` + 40+ MapTiler events
...
- Throws if not within MapTiler component
...
- `CreateMapTilerActions` - Map creation actions interface
- `MapTilerActions` - Extended actions with registration
- `MapTilerMethods` - Full map manipulation methods
...
### Test Coverage (241 tests across 35 files)
...
- **deploy-docs.yml**: Deploy VitePress docs to Cloudflare Pages (`vue-maptiler-gl.pages.dev`) via `wrangler-action`; cache headers from `docs/public/_headers`
...
- `shallowRef` and `markRaw` keep MapTiler objects out of Vue's reactive graph
...
1. **Expressions**: No IDE type hints for MapTiler expressions
2. **Style Functions**: Limited JS function support (use expressions)
3. **Private APIs**: Some MapTiler SDK internals may change
