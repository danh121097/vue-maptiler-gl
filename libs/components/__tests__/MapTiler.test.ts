...
import type { MapTilerActions } from '@libs/types';
import type { MockMap } from '../../__tests__/mock-maptiler';

vi.mock('@maptiler/sdk', async (importOriginal) => {
  const { mockMapTilerModule } = await import('../../__tests__/mock-maptiler');
  return mockMapTilerModule(importOriginal as () => Promise<any>);
});
...
const MapTiler = (await import('../MapTiler.vue')).default as Component;
...
/**
 * Mounts `<MapTiler>` with reactive options and waits for the map to exist.
 ...
 */
async function mountMapTiler(
  options: Partial<MapOptions>,
  onError?: (error: unknown) => void,
) {
  const props = shallowReactive({ options });
  let actions: MapTilerActions | null = null;
        ...
        return () =>
          h(
            MapTiler,
            {
              ...
              onError,
              register: (registered: MapTilerActions) => {
...
describe('<MapTiler> option watchers', () => {
  it('creates the map once the container is mounted', async () => {
    const { getMap } = await mountMapTiler({
      style: STYLE,
      center: [0, 0],
      zoom: 1,
    });
  ...
  it('moves the map when :options.center changes after mount', async () => {
    const { props, getMap } = await mountMapTiler({
      style: STYLE,
      center: [0, 0],
      zoom: 1,
    });
...
describe('<MapTiler> setMapOptions', () => {
  it('lets later :options changes through for keys it did not override', async () => {
    const { props, getMap, getActions } = await mountMapTiler({
      style: STYLE,
      center: [0, 0],
      zoom: 1,
    });
  ...
  it('does not re-issue setStyle for an object style it did not change', async () => {
    ...
    const { getMap, getActions } = await mountMapTiler({
      style,
      center: [0, 0],
      zoom: 1,
    });
...
describe('<MapTiler> error state', () => {
  it('keeps the default slot mounted through a runtime map error', async () => {
    const onError = vi.fn();
    const { getMap, hasChild } = await mountMapTiler(
