...
/**
 * Enhanced MapTiler component props with comprehensive configuration options
 */
interface MapTilerProps {
  /** Map configuration options */
  options?: Partial<MapOptions>;
  /** Callback for registering map actions */
  register?: (actions: MapTilerActions) => void;
...
const props = withDefaults(defineProps<MapTilerProps>(), {
  ...
  debug: false,
  autoCleanup: true,
  containerId: () => `maptiler-${Math.random().toString(36).substring(2, 11)}`,
  containerClass: '',
});
...
} = useCreateMapTiler(mapContainerRef, styleRef, {
  ...unref(mapOptions),
...
<template>
  <div
    :id="containerId"
    ref="mapContainerRef"
    :class="['maptiler-container', containerClass]"
  >
    <!-- Loading state -->
