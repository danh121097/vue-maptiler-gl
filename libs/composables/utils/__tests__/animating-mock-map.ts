...
/**
 * `MockMap` plus the camera methods, modelling the two MapTiler behaviours the
...
export class AnimatingMockMap extends MockMap {
  ...
  /** Whether an ease is in flight — mirrors MapTiler's `isMoving()`. */
  isMoving(): boolean {
    return this.inflight !== null;
  }

  /** Ends the in-flight ease the way MapTiler's `_afterEase` does. */
  complete(): void {
