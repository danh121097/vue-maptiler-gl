...
import type { CameraOptions } from '@maptiler/sdk';

/** Property MapTiler copies from `eventData` onto every movement event it emits. */
...
export function createCameraAnimation(
   ...
   * the promise. MapTiler starts `easeTo` / `flyTo` by stopping any in-flight
   ...
   *
   * Map `error` events are not treated as animation failures: MapTiler fires
