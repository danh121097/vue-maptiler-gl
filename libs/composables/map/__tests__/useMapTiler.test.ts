...
import { useMapTiler } from '../useMapTiler';

/**
 * The narrow slice of `MapTilerActions` that `register` reads. The rest is
 * setter plumbing `useMapTiler` only forwards, never inspects.
 */
function actionsFor(map: ReturnType<typeof shallowRef<Map | null>>) {
  ...
  } as unknown as MapTilerActions;
}

describe('useMapTiler listener hygiene', () => {
  it('does not leave a load listener on a map it has moved off', async () => {
    ...
    const { register } = withSetup(() => useMapTiler());
  ...
  it('reports the map ready once the deferred load event arrives', async () => {
    ...
    const { register, mapStatus, isMapReady } = withSetup(() => useMapTiler());
    await register(actionsFor(map));
  ...
  it('takes an already-loaded map as ready without waiting for an event', async () => {
    ...
    const { register, isMapReady } = withSetup(() => useMapTiler());
    await register(actionsFor(map));
...
describe('useMapTiler creation failure', () => {
  it('reports hasMapError when the map could not be constructed', async () => {
    // Mirrors `useCreateMapTiler` on a host without WebGL: `register` runs
    ...
    const { register, hasMapError, mapStatus } = withSetup(() => useMapTiler());
