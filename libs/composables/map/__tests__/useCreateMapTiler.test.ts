...
vi.mock('@maptiler/sdk', async (importOriginal) => {
  const { mockMapTilerModule, MockMap } = await import(
    '../../../__tests__/mock-maptiler'
  );
  const mod = await mockMapTilerModule(importOriginal as () => Promise<any>);
...
const { useCreateMapTiler } = await import('../useCreateMapTiler');

function setupMap(onError?: (error: any) => void) {
  const el = shallowRef<HTMLElement | null>(null);
  const style = ref('https://example.test/style.json');
  const actions = withSetup(() => useCreateMapTiler(el, style, { onError }));
  return { el, actions };
}
...
describe('useCreateMapTiler reactivity contract', () => {
  ...
  it('hands the register callback live refs', async () => {
    ...
    const actions = withSetup(() => useCreateMapTiler(el, style, { register }));
...
describe('useCreateMapTiler error classification', () => {
