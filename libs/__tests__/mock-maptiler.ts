...
/**
 * Minimal stand-in for `@maptiler/sdk`'s `Map`.
 *
 * MapTiler needs a real WebGL context, which happy-dom does not provide, so
...
export class MockMap {
  ...
  /**
   * Layer-scoped handlers, keyed `event::layerId`. MapTiler's three-argument
   ...
   * caller passed. MapTiler's `Evented` keeps one-time listeners in their own
  ...
  /**
   * Tracked separately from `isLoaded`: MapTiler's `loaded()` also waits on
  ...
  /**
   * Splits MapTiler's overloaded signature: `(type, handler)` subscribes to the
...
/**
 * `vi.mock` factory that keeps every real @maptiler/sdk export except `Map`.
 * The library reads `getVersion()` from the real module, so a blanket mock
 * would break unrelated helpers.
 */
export async function mockMapTilerModule(
